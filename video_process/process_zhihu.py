...
def process_zhihu_video_excel(excel_file="tangzong.xlsx"):
    prefix = "https://lens.zhihu.com/api/v4/videos/"
    df = pd.read_excel(excel_file)
    res = dict()
    fw = open("tangzong_url.txt", "a")
    for i in range(df.shape[0]):
        try:
            id = df.iloc[i]["vid"]
            page_url = df.iloc[i]["zhihu_url"]
            html_text = requests.get(page_url).text
            vid_begin_idx = html_text.find("videoId") + 10
            vid_end_idx = html_text[vid_begin_idx:].find('"') + vid_begin_idx
            vid = html_text[vid_begin_idx:vid_end_idx]
...
def load_id2url(filename="zhihu_0_520_url.txt"):
    prefix = "https://lens.zhihu.com/api/v4/videos/"
    res = dict()
    lines = open(filename)
    for l in lines:
