...
from translator import translate_srt_dir_with_context_zh2en
...
def prep_hw_data():
    ...
    # video_dir_list.append("/Users/tal/work/lingtok_server/video_process/hw/videos/短剧/8147-我被套路撞了N下腰-06-01-君-富士山下")
    video_dir_list.append("/Users/tal/work/lingtok_server/video_process/hw/videos/短剧/0110-8233-被偷走爱的那十年--君-Easylove43-01")
    ...
    for video_dir in video_dir_list:
        ...
        # series_name = "拼音规则"
        # series_name = "声母歌"
        # series_name = "2024版新教材拼音朗读视频全集"
        # series_name = "声母练习"
        # series_name = "我被套路撞了N下腰"
        # series_name = "被偷走爱的那十年"
        # For debug
        skip_srt = True
        skip_quiz = False
        ...
        if not skip_quiz:
            # translate_srt_dir_with_context_zh2en(srt_dir, refine_ar=True, refine_py=True)
            ...
            from recommender.quiz_generator import CharFillingQuizGeneratingWorker, QuizGeneratingCtx, CharPinyinQuizGeneratingWorker
            ...
            char_quiz_worker = CharPinyinQuizGeneratingWorker(quiz_worker_config)

            import pdb; pdb.set_trace()
            for i in tqdm(range(len(df_list))):
                df_list[i][0] = os.path.join(video_dir, df.iloc[i]["FileName"].split("\\")[-1].strip())
                df_list[i][3] = os.path.join(srt_dir, df_list[i][3].split("\\")[-1].strip())
                df_list[i][4] = os.path.join(srt_dir, df_list[i][4].split("\\")[-1].strip())
                df_list[i][5] = os.path.join(srt_dir, df_list[i][5].split("\\")[-1].strip())
                df_list[i][6] = os.path.join(srt_dir, df_list[i][6].split("\\")[-1].strip())

                zh_srt_file = df_list[i][3]
                ori_word_list = content_tagger.split_srt_words(zh_srt_file, use_ner=False)
                # 按词的长度倒序排列
                ori_word_list.sort(key=len, reverse=True)
                for word in ori_word_list:
                    if content_tagger.tag_word_video_hsklevel(word) <=5:
                        quiz_ctx.extracted_word = word
                        break
                if quiz_ctx.extracted_word == None:
                    import random as rd
                    rd.shuffle(ori_word_list)
                    quiz_ctx.extracted_word = ori_word_list[0]
                try:
                    if len(quiz_ctx.extracted_word) > 1:
                        quiz_res = word_quiz_worker.action(quiz_ctx)
                    else:
                        quiz_res = char_quiz_worker.action(quiz_ctx)
                    content = word_quiz_worker.to_dict(quiz_res)
                
                    content["vid"] = "{}_{}".format(i, df_list[i][1])
