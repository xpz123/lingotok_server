...
def trans_word_to_en_with_mapcsv(ori_csv_file, en_csv_file):
    ...
    for i in range(df.shape[0]):
        clean_word = df.iloc[i]["单词"].strip().split("（")[0].strip()
        if df.iloc[i]["是否正确"] == "正确":
            zh_en_map[clean_word] = df.iloc[i]["英语翻译"].strip().lower()
        else:
            if not pd.isna(df.iloc[i]["修改结果"]):
                zh_en_map[clean_word] = df.iloc[i]["修改结果"].strip()
            else:
                continue
...
def generate_trans_quiz(en_ar_csv, quiz_csv, quiz_jsonl):
    def get_distractor_words(content_tagger):
...
def tag_level(vod_csv, level_csv):
    # 读取原始CSV文件
    df = pd.read_csv(vod_csv)
    ...
    # 创建word到level的映射
    word2level = dict()
    hsk_df = pd.read_csv("/Users/tal/work/lingtok_server/video_process/hsk_dictionary/HSK_zh_en_ar.csv")
    for i in tqdm(range(hsk_df.shape[0])):
        word = hsk_df.iloc[i]["单词"].strip().split("（")[0]
        level_str = hsk_df.iloc[i]["单词"].strip().split("（")[-1].replace("）", "").strip()
        try:
            word2level[word] = level_dict[level_str]
        except Exception as e:
            print (e)
            print ("{} error".format(word))
    ...
    for i in tqdm(range(df.shape[0])):
        word = df.iloc[i]["单词"]
        level = word2level.get(word, 6)  # 如果找不到对应的level,默认为6级
        levels.append(level)
    ...
    # 保存到新的CSV文件
    df.to_csv(level_csv, index=False)
