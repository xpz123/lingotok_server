...
def prep_hw_data():
    ...
    for video_dir in video_dir_list:
        ...
        if not skip_quiz:
            ...
            from recommender.quiz_generator import ShengYunMuQuizGeneratingWorker, QuizGeneratingCtx
            ...
            quiz_worker = ShengYunMuQuizGeneratingWorker(quiz_worker_config)
            import pdb; pdb.set_trace()
            for i in tqdm(range(len(df_list))):
                if df_list[i][1].find("2024版新教材拼音朗读视频全集") != -1:
                    title = df_list[i][1]
                    sub_titles = title.split("-")
                    syms = sub_titles[2].strip().replace("（朗读）", "").strip().split(" ")[1:]
                    new_title = sub_titles[0].strip()+ "_" + sub_titles[1].strip() + "_" + "-".join(syms)
                    print (new_title)
                    df_list[i][1] = new_title
                elif df_list[i][1].find("5. 声母练习_【合集】【普通话声母学习】") != -1:
                    df_list[i][1] = df_list[i][1].split("_")[0].strip() + "_" + df_list[i][1].split("-")[-1].strip()
                    print (df_list[i][1])
                sym = df_list[i][1].split("_")[-1].split("-")[0].strip()
                df_list[i][1] = df_list[i][1].replace("_modified", "")
                quiz_ctx.shengyunmu = sym
                try:
                    quiz_res = quiz_worker.action(quiz_ctx)
                    content = quiz_worker.to_dict(quiz_res)
                
                    content["vid"] = "{}_{}".format(i, sym)
