...
def calculate_silence_ratio(video_path, silence_threshold=0.01):
    ...
    if audio is None:
        print("视频没有音频流")
        return 1.0  # 如果没有音频，认为100%静音
    
    # 获取音频数据
    audio_array = audio.to_soundarray(fps=22000)
    
    # 计算每帧的平均音量
    avg_volume = np.abs(audio_array).mean(axis=1)
    
    # 计算静音时长
    silent_frames = avg_volume < silence_threshold
    total_frames = len(avg_volume)
    
    # 计算静音帧数和总时长
    silent_duration = np.sum(silent_frames) / audio.fps  # 静音时长（秒）
    total_duration = clip.duration  # 视频总时长（秒）
...
def translate_quiz_metainfo(metainfo_filename, new_metainfo_filename):
    ...
    lines = open(metainfo_filename, "r", encoding="utf-8").readlines()
...
class VideoProcessor:
    def __init__(self):
        hsk_word_list = list()
        with open("hsk_dictionary/hsk-level6.txt", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if line == "":
                    continue
    ...
    def generate_zhsrt(self, play_url, file_name, audio_path=None, gen_ar=True, gen_en=True, audio_text=None, gen_pinyin=True):
        res = {"zh_srt": "{}_Chinese.srt".format(file_name)}
        zh_srt_fw = open("{}_Chinese.srt".format(file_name), "w", encoding="utf-8")
        if gen_ar:
            ar_srt_fw = open("{}_Arabic.srt".format(file_name), "w", encoding="utf-8")
            res["ar_srt"] = "{}_Arabic.srt".format(file_name)
        if gen_en:
            en_srt_fw = open("{}_English.srt".format(file_name), "w", encoding="utf-8")
    ...
    def compress_video(self, video_file, compressed_video_file, too_high_resolution=480, compress_ratio=1.2):
        ...
        if w * h > multi_resolution:
            ...
            if new_h % 2 != 0:
                new_h += 1
            cmd = "ffmpeg -y -loglevel error -i {} -vf scale={}:{} {}".format(video_file.replace(" ", "\\ ").replace("&", "\\&"), new_w, new_h, compressed_video_file.replace(" ", "\\ "))
            os.system(cmd)
            return True
        return  False
...
if __name__ == "__main__":
    print (calculate_silence_ratio("/Users/tal/work/lingtok_server/tmp_audio/bad.mp4"))
    # video_processor = VideoProcessor()
    # video_processor.generate_quiz_zh_tiankong("/Users/tal/work/lingtok_server/video_process/huoshan/萌宠/爱撒娇的淘气喵/srt_dir/0be1e906-71f3-42a5-8323-037494741a8f_Chinese.srt")
