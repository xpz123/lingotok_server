...
def call_convert(asset_id, ignore_sub=True):
    try:
        ...
        if ignore_sub:
            request.body = AssetProcessReq(
                subtitle_id=[6],
                asset_id=asset_id,
                template_group_name="adaptive_template_group"
            )
        else:
            request.body = AssetProcessReq(
                asset_id=asset_id,
...
def upload_media(video_path, zh_srt_path=None, en_srt_path=None, ar_srt_path=None, py_srt_path=None, cover_path=None, title=None, description=None):
    # create asset
    video_name = video_path.split("/")[-1].replace(".mp4", "")[:50]
    try:
...
if __name__ == "__main__":
    df = pd.read_csv("hw_resub.csv")
    for i in tqdm(range(350, df.shape[0])):
        asset_id = df.iloc[i]["asset_id"]
        call_convert(asset_id)
        # if i % 30 == 0:
        #     sleep(10)
    # pass
