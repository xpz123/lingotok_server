...
def generate_subtitle(sents_csv, srt_prefix):
...
if __name__ == '__main__':
    ori_file = "Dr. Asmac材料1/合成文本/lesson1-part2.txt"
    root_dir = ori_file.replace(".txt", "")
    if not os.path.exists(root_dir):
        os.makedirs(root_dir)
    ori_text = open(ori_file).readline().strip()
    ...
    create_video(sents_file, image_dir, audio_dir, video_dir)
    ...
    # add_subtitles_to_video(final_video_path, zh_srt, ar_srt, srt_video_path)